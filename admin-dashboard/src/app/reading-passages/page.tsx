...
import { Plus, Search, BookOpen, Trash2, Edit2, Archive } from "lucide-react";
...
export default function ReadingPassagesPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    `/admin/reading-passages?page=${page}&limit=${limit}&search=${search}`,
    fetcher
  );
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("هل أنت متأكد من أرشفة/حذف هذه القطعة؟")) return;
    try {
    ...
    <div className="flex min-h-screen bg-slate-50" dir="rtl">
      ...
      <main className="flex-1 p-8">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">القطع القرائية</h1>
            <p className="text-slate-500 mt-2">
              إدارة النصوص والفقرات الخاصة بأسئلة الاستيعاب.
            </p>
          </div>
          <Link
            href="/reading-passages/new"
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6 flex gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="ابحث عن قطعة قرائية..."
              value={search}
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 text-red-700 p-4 rounded-lg">
            حدث خطأ أثناء تحميل القطع القرائية.
          </div>
        ) : passages.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
            <BookOpen className="w-12 h-12 text-slate-300 mx-auto mb-4" />
            <p className="text-slate-500">لا توجد قطع قرائية مطابقة للبحث.</p>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <table className="w-full text-right">
              <thead className="bg-slate-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold text-slate-600">الصعوبة</th>
                ...
                {passages.map((p: any) => (
                  <tr key={p.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{p.title || "بدون عنوان"}</div>
                      <div className="text-sm text-slate-500 truncate max-w-xs">{p.passageText}</div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2 justify-end">
                        <Link href={`/reading-passages/${p.id}/edit`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">
            ...
            <div className="p-4 border-t flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <span className="px-3 py-1">{page} / {meta.pages}</span>
                <button
